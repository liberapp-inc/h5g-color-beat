...
class Button extends GameObject{
    ...
    constructor( text:string, fontsize:number, fontRGB:number, xRatio:number, yRatio:number, wRatio:number, hRatio:number, rgb:number, alpha:number, line:boolean, onTap:()=>void ) {
        super();

        let shape = new egret.Shape();
        GameObject.baseDisplay.addChild(shape);
        if( line )  shape.graphics.lineStyle( 2, fontRGB );
